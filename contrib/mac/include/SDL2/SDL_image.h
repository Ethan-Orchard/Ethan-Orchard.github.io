...
/**
 * Load an image from a filesystem path into a GPU m_texture.
 *
 ...
 * loading it.
 *
 * If the loaded image has transparency or a colorkey, a m_texture with an alpha
 ...
 *
 * When done with the returned m_texture, the app should dispose of it with a
 * call to SDL_DestroyTexture().
 *
 * \param renderer the SDL_Renderer to use to create the GPU m_texture.
 * \param file a path on the filesystem to load an image from.
 * \returns a new m_texture, or NULL on error.
 *
 * \since This function is available since SDL_image 2.0.0.
 *
...
/**
 * Load an image from an SDL data source into a GPU m_texture.
 *
 * An SDL_Texture represents an image in GPU memory, usable by SDL's 2D Render
 * API. This can be significantly more efficient than using a CPU-bound
 * SDL_Surface if you don't need to manipulate the image directly after
